...
class ProductionNonconformityExport implements WithEvents, WithTitle
{
    ...
    public function title(): string { return 'Nonconformity'; }
            ...
            AfterSheet::class => function (AfterSheet $event) {
                ...
                foreach ($this->reports as $report) {
                    ...
                    foreach ($report->details as $detail) {
                        ...
                        $sheet->setCellValue("D{$row}", $detail->occurrence_time ?? '-');
                        ...
                        $sheet->setCellValue("I{$row}", $detail->unit ?? '-');
                        $sheet->setCellValue("J{$row}", $detail->hazard_category ?? '-');
                        $sheet->setCellValue("K{$row}", $detail->disposition ?? '-');
                        $sheet->setCellValue("L{$row}", $detail->remark ?? '-');

                        // Kolom G (Ketidaksesuaian) rata kiri karena teks panjang
                ...
                // Kolom G & L kasih lebar fixed supaya tidak terlalu lebar
