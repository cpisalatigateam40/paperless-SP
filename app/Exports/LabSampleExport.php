...
class LabSampleExport implements WithEvents, WithTitle
{
    ...
    public function title(): string { return 'Lab Sample'; }
            ...
            AfterSheet::class => function (AfterSheet $event) {
                ...
                foreach ($this->reports as $report) {
                    ...
                    foreach ($report->details as $detail) {
                        ...
                        $sheet->setCellValue("J{$row}", $report->storage ?? '-');
                        $sheet->setCellValue("K{$row}", $detail->sample_type ?? '-');
                        $sheet->setCellValue("L{$row}", $detail->quantity ?? '-');
                        $sheet->setCellValue("M{$row}", $detail->unit ?? '-');
                        $sheet->setCellValue("N{$row}", $detail->notes ?? '-');
