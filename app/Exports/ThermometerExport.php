...
class ThermometerExport implements WithEvents, WithTitle
{
    ...
    public function title(): string { return 'Thermometer'; }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A1', 'LAPORAN VERIFIKASI THERMOMETER');
                ...
                foreach ($this->reports as $report) {
                    ...
                    foreach ($report->thermometerDetails as $detail) {
                        $byIndex = $detail->measurements->groupBy('inspection_time_index');
                        $p1 = $byIndex->get(1, collect())->keyBy('standard_temperature');
                        $p2 = $byIndex->get(2, collect())->keyBy('standard_temperature');

                        $fmt = fn($t) => ($p1->get($t)?->measured_value ?? '-')
                            . ' / '
                            . ($p2->get($t)?->measured_value ?? '-');
                        ...
                        $sheet->setCellValue("G{$row}",
                            ($detail->thermometer->type ?? '-') . ' - ' . ($detail->thermometer->code ?? '-'));
                        $sheet->setCellValue("H{$row}", $fmt(0));
                        $sheet->setCellValue("I{$row}", $fmt(100));
                        $sheet->setCellValue("J{$row}", $detail->notes ?? $detail->note ?? '-');
