...
class AlatVerificationExport implements WithEvents, WithTitle
{
    ...
    public function title(): string { return 'Verifikasi Alat'; }
            ...
            AfterSheet::class => function (AfterSheet $event) {
                ...
                foreach ($this->reports as $report) {
                    ...
                    foreach ($report->details as $detail) {
                        $jenisAlat = $detail->alat_type === 'scale' ? 'Timbangan' : 'Thermometer';
                        $kodeAlat  = ($detail->alat->type ?? '-') . ' - ' . ($detail->alat->code ?? '-');

                        $sheet->setCellValue("A{$row}", $no);
                        $sheet->setCellValue("B{$row}", Carbon::parse($report->date)->format('d/m/Y'));
                        $sheet->setCellValue("C{$row}", $shiftNum ?: ($report->shift ?? '-'));
                        $sheet->setCellValue("D{$row}", $shiftGroup ?: '-');
                        $sheet->setCellValue("E{$row}", $report->created_by ?? '-');
                        $sheet->setCellValue("F{$row}", $jenisAlat);
                        $sheet->setCellValue("G{$row}", $kodeAlat);
                        $sheet->setCellValue("H{$row}", $detail->titik_ukur ?? '-');
                        $sheet->setCellValue("I{$row}", $detail->nilai_baca ?? '-');
                        $sheet->setCellValue("J{$row}", $detail->notes ?? '-');
