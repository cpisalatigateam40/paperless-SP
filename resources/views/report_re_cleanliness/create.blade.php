...
<div class="container-fluid">
    <div class="card shadow mb-4">
        ...
        <div class="row mb-3">
            <div class="col-md-4">
                <label>Tanggal</label>
                <input type="date" name="date" class="form-control mb-5" value="{{ \Carbon\Carbon::today()->toDateString() }}" required>
            </div>
        </div>
                        ...
                        @foreach ($room->elements as $element)
                            <div class="col-md-4 mb-4">
                                ...
                                <div class="cleanliness-fields mt-2" style="display: none;">
                                    ...
                                    <input name="rooms[{{ $room->uuid }}][elements][{{ $element->uuid }}][corrective_action]"
                                        class="form-control mb-2 corrective_action" placeholder="Tindakan koreksi">
                                    <input name="rooms[{{ $room->uuid }}][elements][{{ $element->uuid }}][verification]"
                                        class="form-control verification" placeholder="Verifikasi setelah koreksi">
                                </div>
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="tab-pane fade" id="equipmentTab">
            ...
            <div class="mb-3 border cleanliness-item">
                ...
                <div class="form-wrapper" style="display: none; margin-top: 1rem; padding: 1rem;">
                    <input type="hidden" name="equipments[{{ $equipment->uuid }}][name]" value="{{ $equipment->name }}">
                    <div class="row">
                        @foreach ($equipment->parts as $part)
                            <div class="col-md-4 mb-4">
                                ...
                                <div class="cleanliness-fields mt-2" style="display: none;">
                                    <input name="equipments[{{ $equipment->uuid }}][parts][{{ $part->uuid }}][notes]"
                                        class="form-control mb-2 notes" placeholder="Catatan jika kotor">
                                    <input name="equipments[{{ $equipment->uuid }}][parts][{{ $part->uuid }}][corrective_action]"
                                        class="form-control mb-2 corrective_action" placeholder="Tindakan koreksi">
                                    <input name="equipments[{{ $equipment->uuid }}][parts][{{ $part->uuid }}][verification]"
                                        class="form-control verification" placeholder="Verifikasi setelah koreksi">
                                </div>
                            </div>
                        @endforeach
