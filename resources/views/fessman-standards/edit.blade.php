...
<div class="container">
    <div class="card shadow">
        <div class="card-header">
            <h4>Edit Standard Fessman</h4>
        </div>

        <div class="card-body">
            <form action="{{ route('fessman-standards.update', $standard->uuid) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        <label class="form-label">Time (menit) Min</label>
                        <input type="number" name="time_minute_min" class="form-control"
                            value="{{ old('time_minute_min', $standard->time_minute_min) }}">
                    </div>
                    <div class="col-md-3 mb-3">
                        <label class="form-label">Time (menit) Max</label>
                        <input type="number" name="time_minute_max" class="form-control"
                            value="{{ old('time_minute_max', $standard->time_minute_max) }}">
                    </div>
                </div>
                ...
                <a href="{{ route('fessman-standards.index') }}" class="btn btn-secondary">Kembali</a>
            </form>
        </div>
    </div>
</div>
