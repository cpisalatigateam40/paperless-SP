        ...
        <tbody>
            @foreach($details as $i => $detail)
                ...
                <tr>
                    ...
                    <td><input type="number" step="0.01" name="thermo_data[{{ $i }}][p1_0]" class="form-control" value="{{ $m1[0]->measured_value ?? '' }}" {{ $isEdit ? '' : '' }}></td>
                    <td><input type="number" step="0.01" name="thermo_data[{{ $i }}][p1_100]" class="form-control" value="{{ $m1[100]->measured_value ?? '' }}" {{ $isEdit ? '' : '' }}></td>
                    <td><input type="number" step="0.01" name="thermo_data[{{ $i }}][p2_0]" class="form-control" value="{{ $m2[0]->measured_value ?? '' }}" {{ !$isEdit ? '' : '' }}></td>
                    <td><input type="number" step="0.01" name="thermo_data[{{ $i }}][p2_100]" class="form-control" value="{{ $m2[100]->measured_value ?? '' }}" {{ !$isEdit ? '' : '' }}></td>
                    <td>
