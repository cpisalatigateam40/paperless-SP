        ...
        <tbody>
            @foreach($details as $i => $detail)
                ...
                <tr>
                    ...
                    <td><input type="number" step="0.01" name="data[{{ $i }}][p1_1000]" class="form-control"  value="{{ optional($m1->get(1000))->measured_value }}" {{ $isEdit ? '' : '' }}></td>
                    <td><input type="number" step="0.01" name="data[{{ $i }}][p1_5000]" class="form-control" value="{{ optional($m1->get(5000))->measured_value }}" {{ $isEdit ? '' : '' }}></td>
                    <td><input type="number" step="0.01" name="data[{{ $i }}][p1_10000]" class="form-control" value="{{ optional($m1->get(10000))->measured_value }}" {{ $isEdit ? '' : '' }}></td>
                    <td><input type="number" step="0.01" name="data[{{ $i }}][p2_1000]" class="form-control" value="{{ optional($m2->get(1000))->measured_value }}" {{ !$isEdit ? '' : '' }}></td>
                    <td><input type="number" step="0.01" name="data[{{ $i }}][p2_5000]" class="form-control" value="{{ optional($m2->get(5000))->measured_value }}" {{ !$isEdit ? '' : '' }}></td>
                    <td><input type="number" step="0.01" name="data[{{ $i }}][p2_10000]" class="form-control" value="{{ optional($m2->get(10000))->measured_value }}" {{ !$isEdit ? '' : '' }}></td>
                    <td>
...
document.addEventListener('DOMContentLoaded', function () {
    const time2Val = document.getElementById('scale-time2').value;
    document.querySelectorAll('.time2-hidden').forEach(input => input.value = time2Val);
});

document.getElementById('scale-time2').addEventListener('input', function () {
