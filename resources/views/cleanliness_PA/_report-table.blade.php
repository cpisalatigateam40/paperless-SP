        ...
        @forelse($filteredReports as $report)
            <tr>
                ...
                <td>
                    ...
                    <form action="{{ route('process-area-cleanliness.destroy', $report->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Yakin ingin menghapus laporan ini?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-sm btn-danger">Hapus</button>
                    </form>

                    <a href="{{ route('process-area-cleanliness.export.pdf', $report->uuid) }}" target="_blank" class="btn btn-sm btn-outline-secondary">
                        🖨 Cetak PDF
                    </a>

                    @can('approve report')
                        @if(!$report->approved_by)
                            <form action="{{ route('process-area-cleanliness.approve', $report->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Setujui laporan ini?')">
                                @csrf
            ...
            <tr class="collapse" id="detail-{{ $report->id }}">
                <td colspan="5">
                    ...
                    <div class="d-flex justify-content-end">
                        <a href="{{ route('process-area-cleanliness.detail.create', $report->id) }}" class="btn btn-sm btn-primary mt-3">
