                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <form action="{{ route('report-solvents.destroy', $report->uuid) }}" method="POST"
                                class="d-inline-block" onsubmit="return confirm('Hapus laporan ini?')">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm btn-danger">Hapus</button>
                            </form>

                            <a href="{{ route('report-solvents.export-pdf', $report->uuid) }}" target="_blank"
                            ...
                            <form action="{{ route('report-solvents.approve', $report->id) }}" method="POST"
                            ...
                            <table class="table table-sm table-bordered mb-0 align-middle text-center">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {!! $detail->reverification_action == '1' ? '✔' :
                                            ($detail->reverification_action == '0' ? '✘' : '-') !!}
                                        </td>
                                    ...
                                    <tr class="table-secondary">
                                        <td></td>
                                        <td colspan="5" class="text-start">↳ Koreksi Lanjutan #{{ $index + 1 }}</td>
                                        <td>{{ $followup->notes ?? '-' }}</td>
                                        <td>{{ $followup->corrective_action ?? '-' }}</td>
                                        <td>{!! $followup->verification == '1' ? '✔' : '✘' !!}</td>
                                    </tr>
                                    @endforeach
                                    @endforeach
                                </tbody>
