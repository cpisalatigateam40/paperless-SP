...
<div class="container-fluid">
    ...
    <form action="{{ route('report_fessman_cookings.update', $report->uuid) }}" method="POST">
        ...
        @php
            ...
            $coolingSteps = [
                ['name' => 'AIR COOLING WITH SHOWER INTER 1', 'fields' => ['time_minutes_1', 'time_minutes_2', 'rh_1', 'rh_2']],
                ['name' => 'BLOWER SHOWER OUT SECTION 2',     'fields' => ['rh_1', 'rh_2']],
                ['name' => 'AIR COOLING WITH SHOWER INTER 2', 'fields' => ['time_minutes_1', 'time_minutes_2', 'rh_1', 'rh_2']],
                ['name' => 'OUT TRANSPORT',                   'fields' => ['rh_1', 'rh_2']],
                ['name' => 'SUHU PRODUK KELUAR',              'fields' => ['product_temp_after_exit_1', 'product_temp_after_exit_2', 'product_temp_after_exit_3', 'avg_product_temp_after_exit']],
            ];
        @endphp
                    ...
                    <div class="card-body row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="details[{{ $i }}][sensory_check][can_be_twisted]" class="form-control">
                                <option value="">-- Pilih --</option>
                                <option value="1" {{ optional($detail?->sensoryCheck)->can_be_twisted == 1 ? 'selected' : '' }}>Bisa</option>
                                <option value="0" {{ optional($detail?->sensoryCheck)->can_be_twisted == 0 ? 'selected' : '' }}>Tidak Bisa</option>
                            </select>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered small" style="table-layout:fixed; width:100%;">
                                ...
                                <tbody>
                                    ...
                                    @php
                                        $stepData = $detail ? $detail->coolingDowns->firstWhere('step_name', $step['name']) : null;
                                    @endphp
                                    <tr>
                                        <td>
                                            <input type="text" readonly class="form-control form-control-sm"
                                        ...
                                        @foreach(['time_minutes_1', 'time_minutes_2', 'rh_1', 'rh_2', 'product_temp_after_exit_1', 'product_temp_after_exit_2', 'product_temp_after_exit_3', 'avg_product_temp_after_exit', 'raw_weight', 'cooked_weight', 'loss_kg', 'loss_percent'] as $field)
                                        <td>
                                            @if(in_array($field, $step['fields']))
                                            <input type="number" step="any" class="form-control form-control-sm"
