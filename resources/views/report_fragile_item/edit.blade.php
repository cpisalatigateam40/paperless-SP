...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header">
            <h5 class="mb-3">Edit Pemeriksaan Barang Mudah Pecah (Glass & Brittle Plastic)</h5>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle">
                                <input type="hidden" name="items[{{ $item->uuid }}][time_start]"
                                    value="{{ (optional($detail)->time_start ?? 0) }}">
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.check-all').forEach(function(checkAllBox) {
        checkAllBox.addEventListener('change', function() {
            ...
            document.querySelectorAll('.' + sectionClass).forEach(cb => cb.checked = checkAllBox
                .checked);
        });
    });
    ...
    document.querySelectorAll('.check-all-time-start').forEach(function(checkAllBox) {
        checkAllBox.addEventListener('change', function() {
            ...
            document.querySelectorAll('.check-time-start-' + section).forEach(cb => cb.checked =
                checkAllBox.checked);
        });
    });
    ...
    document.querySelectorAll('.check-all-time-end').forEach(function(checkAllBox) {
        checkAllBox.addEventListener('change', function() {
            const section = this.dataset.section;
            document.querySelectorAll('.check-time-end-' + section).forEach(cb => cb.checked =
                checkAllBox.checked);
        });
    });
});
